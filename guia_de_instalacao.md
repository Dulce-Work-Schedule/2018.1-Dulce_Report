...
```bash
$ cd 2018.1-Dulce_Report/Docker/Dev
```
```bash
...
```bash
$ sudo docker-compose -f start-report.yml up -d
```
...
Inicialmente, é necessário fazer os seguintes comandos:
...
$ ./script_coveralls.sh
```
Em seguida, para rodar os testes:

```bash
$ cd 2018.1-Dulce_Report/Docker/Test
```
```bash
$ sudo docker-compose -f build.yml  build
```
